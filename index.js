...
function createMood(currentMood) {
    ...
    currentTime = currentTime.toLocaleTimeString() + " " + currentDate.toLocaleDateString();
	
	if (currentMood == "happy" || currentMood == "cheerful" || currentMood == "joyful" || currentMood == "content") {
		happy.push(currentTime);
	}
	else if (currentMood == "sad" || currentMood == "depressed" || currentMood == "unhappy" || currentMood == "down") {
		sad.push(currentTime);
	}
	else if (currentMood == "angry" || currentMood == "mad" || currentMood == "frustrated" || currentMood == "livid") {
		angry.push(currentTime);
	}
	else if(currentMood == "indifferent" || currentMood == "okay" || currentMood == "alright" || currentMood == "whatever"){
...
const handlers = {
    'LaunchRequest': function () {
        this.response.speak("How are you feeling?").listen("I didn't catch that. How are you feeling?");
        this.emit(':responseReady');
    },
    'GiveMoodIntent': function () {
        var cMood = this.event.request.intent.slots.MoodType.value;
    	createMood(cMood);
    	this.response.speak("Thanks for sharing. Your mood, " + cMood + ", has been logged. You now have " + totalNumMoods + " moods logged.");
        this.emit(':responseReady');
    },
    'GetMoodReview':function()    
    {
    	var percent = findPercentage();
    	var output = "You have been happy "+ percent[0]+" percent of the time. You have been sad "+ percent[1]+
    	" percent of the time. You have been angry "+percent[2]+" percent of the time. You have been indifferent "+
    	percent[3]+" percent of the time.";
